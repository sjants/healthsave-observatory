...
A HealthSave-compatible server does **not** have to be this one. The minimum core contract is: `GET /api/health` returns `2xx`, `GET /api/apple/status` returns flat metric objects, and `POST /api/apple/batch` accepts batches with a `2xx`. Idempotent batch handling is strongly recommended. Optional Observatory v2 sync receipts (`/api/v2/sync/*`) light up extra confidence tiers in the app when present — a v1-only server still syncs cleanly without them. One honest caveat: iOS ≤ 1.7.2 labelled a run where every metric had nothing new to send as "needs retry" when the receipt routes were absent (the confirm stage leaned on them more than documented); 1.8.0 fixes that on the client, and also closes each completed run with an optional `PUT /api/v2/sync/runs/{sync_run_id}/summary` that a v1-only server may simply 404.
